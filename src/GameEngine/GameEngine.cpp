...
void GameEngine::Initialize()
{
	...
	window = SDL_CreateWindow(U8_TO_CHARPTR("2D Engine"),
...
void GameEngine::SetUp()
{
	...
	// Lua绑定C++函数
	registry->GetSystem<ScriptSystem>().LuaBindCpp(*luaState);
}
...
void GameEngine::Input()
{
	...
	while (SDL_PollEvent(&event))
	{
		...
		switch (event.type)
		{
			...
			{
				const auto& keyCode = event.key.keysym.sym;
				eventBus->EmitEvent<KeyReleasedEvent>(keyCode);
				break;
			}
			case SDL_WINDOWEVENT:
			{
				const auto& windowEvent = event.window.event;
				if (windowEvent == SDL_WINDOWEVENT_FOCUS_LOST || windowEvent == SDL_WINDOWEVENT_MINIMIZED)
				{
					timer.pause();
				}
				else if (windowEvent == SDL_WINDOWEVENT_FOCUS_GAINED || windowEvent == SDL_WINDOWEVENT_RESTORED)
				{
					timer.resume();
				}
				break;
...
void GameEngine::Update()
{
	...
	registry->GetSystem<ScriptSystem>().Update(registry, renderer, assetStore, camera);
