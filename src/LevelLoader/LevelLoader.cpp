...
bool LevelLoader::LoadLevel(int level)
{
	...
	if (!loadResult.valid())
	{
		...
		std::string message = U8_TO_CHARPTR("关卡数据加载失败，Lua脚本加载错误：") + std::string(err.what());
		Logger::Instance().Log(LogLevel::ERROR, message);
		return false;
	}
	...
	std::string completeMsg = U8_TO_CHARPTR("关卡数据加载完成：") + std::to_string(level);
	Logger::Instance().Log(LogLevel::INFO, completeMsg);

	{
		...
		for (size_t y = 0; y < mapStyle->rangeY; ++y)
		{
			for (size_t x = 0; x < mapStyle->rangeX; ++x)
			{
				...
				SpriteComponent spriteComponent("jungle", { mapStyle->pixelX, mapStyle->pixelY }, RenderLayer::Terrain, false, { srcRectX, srcRectY });
...
void LevelLoader::LoadEntities(const sol::table& entities, int entitiesNum)
{
	for (int i = 1; i <= entitiesNum; i++)
	{
		...
		if (rigidbody != sol::nullopt)
		{
			...
			rigidBodyComponent.maxSpeed = rigidBodyComponent.speed;
		...
		if (sprite != sol::nullopt)
		{
			...
			// 获取 isDirSheet（bool）
			spriteComponent.isDirSheet = sprite->get<sol::optional<bool>>("isDirSheet").value_or<bool>(false);
		...
		if (projectileEmitter != sol::nullopt)
		{
			...
			projectileEmitterComponent.repeatFrequency = projectileEmitter->get<sol::optional<int>>("repeatFrequency").value_or<int>(1000);
		...
		// 获取ScriptComponent脚本组件并添加到实体
		sol::optional<sol::table> script = components->get<sol::optional<sol::table>>("script");
		if (script != sol::nullopt)
		{
			ScriptComponent scriptComponent;

			// 获取 funcVector（std::vector<sol::function>）
			for(int i = 1; i <= script->size(); i++)
			{
				sol::optional<sol::safe_function> func = script->get<sol::optional<sol::safe_function>>(i);
				if(func != sol::nullopt)
				{
					scriptComponent.funcVec.push_back(func.value());
				}
			}
